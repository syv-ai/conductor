...
A ref stores as its address, `"node.field"`. YAML needs PyYAML (`syv-conductor[yaml]`). What describes a node (`NodeDescription`, `Input`, `Output`, the widgets) is a model too, written for an editor and not read back: call `describe()` again rather than loading a palette.
