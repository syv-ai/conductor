...
def test_validation_errors_not_retried():
    ...
    compiled = compile_graph(Graph(nodes=[GraphNode("n1", "raise-validation", 1, bindings={"text": Static(value="x")})]), reg)
...
def test_node_execution_error_with_retryable_false_not_retried():
    ...
    compiled = compile_graph(Graph(nodes=[GraphNode("n1", "raise-fatal", 1, bindings={"text": Static(value="x")})]), reg)
...
def test_default_node_execution_error_still_retries():
    ...
    compiled = compile_graph(Graph(nodes=[GraphNode("n1", "flaky-exec", 1, bindings={"text": Static(value="x")})]), reg)
