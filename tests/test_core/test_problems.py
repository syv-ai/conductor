...
def test_a_problem_names_a_code_a_message_and_a_place():
    p = Problem(
        code="unknown_input",
        message="Feltet findes ikke på noden.",
        fatal=True,
        node_id="brev",
        field="template",
    )
    ...
    assert p.node_id == "brev"
    assert p.field == "template"


def test_a_problem_about_a_whole_node_names_no_field():
    p = Problem(code="cycle", message="Flowet går i ring.", fatal=True, node_id="brev")
...
def test_every_problem_is_about_a_node():
    ...
    with pytest.raises(TypeError):
        Problem(code="empty", message="Flowet er tomt.", fatal=True)
