...
def test_a_decision_over_a_whole_column_gates_what_hangs_off_the_branch_not_taken():
    """An "if the list is empty" decision: the untaken branch is skipped at the top,
    not an empty series, so a reduction downstream of it does not run."""
    ledger = _ledger([
        DOCS,
        GraphNode(id="decide", type="decide", version=1, bindings={"texts": _edge(("docs", "texts"))}),
        GraphNode(id="full", type="join", version=1, bindings={"texts": _edge(("decide", "full"))}),
        GraphNode(id="empty", type="join", version=1, bindings={"texts": _edge(("decide", "empty"))}),
...
def test_a_gate_at_a_parent_row_reaches_only_the_rows_under_it():
    """An iterating node skipping its series output at row 1 leaves the child
...
def test_a_mask_and_a_gate_differ_only_in_depth():
    """Every row of `up` skipped at its own row: `j` runs on an empty series. `up` skipped
...
def test_two_refs_on_one_index_into_a_scalar_input_read_whichever_covers_the_row():
    """Merging is edges. Per row the one source that covers it; none leaves the row missing; two fail the node."""
    ledger = _ledger([
        DOCS,
...
def test_progress_counts_rows_not_stand_ins():
    """The unit at a shorter row stands in for rows a skip above them never
    let be born; "4 of 10" counts rows, so it is in neither number."""
    ledger = _ledger([
        DOCS,
        GraphNode(id="lines", type="lines", version=1, bindings={"text": _edge(("docs", "texts"))}),
        GraphNode(id="up", type="upper", version=1, bindings={"text": _edge(("lines", "result"))}),
