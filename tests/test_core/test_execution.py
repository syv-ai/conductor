...
class TestStreamingExecution:
    ...
    async def test_diamond_execution(self, three_node_registry):
        ...
        compiled = compile_graph(Graph(nodes=[
                ...
                GraphNode("n4", "combine", 1, bindings={"a": Edges(refs=(Ref('n2', 'result'),)), "b": Edges(refs=(Ref('n3', 'result'),))}),
            ]), three_node_registry)
...
class TestSyncExecution:
    ...
    def test_single_node_no_edges(self, three_node_registry):
        ...
        compiled = compile_graph(Graph(nodes=[GraphNode("n1", "echo", 1, bindings={"text": Static(value="standalone")})]), three_node_registry)
...
class TestErrorHandling:
    async def test_node_execution_error_yields_flow_error(self, registry):
        ...
        compiled = compile_graph(Graph(nodes=[GraphNode("n1", "fail", 1, bindings={"text": Static(value="hello")})]), registry)
...
class TestTimeout:
    async def test_timeout_produces_event(self, registry):
        ...
        compiled = compile_graph(Graph(nodes=[GraphNode("n1", "slow", 1, bindings={"text": Static(value="hello")})]), registry)
