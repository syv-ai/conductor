...
def _build_dep_graph(
    ...
    Managed nodes (compound region body / end) are not directly schedulable,
    so when an edge or consume binding references one as source or target it
    is redirected onto the region's start node — the schedulable proxy that
    actually fires when the region completes.
    """
    ...
    for target_id, entries in compiled.incoming_map.items():
        ...
        for _target_handle, source_id, _source_handle, _edge_id in entries:
            ...
            if effective_source == effective_target:
                # Self-loops via the region start (start → body → end → start)
                # are not real dependencies — drop them.
                continue
